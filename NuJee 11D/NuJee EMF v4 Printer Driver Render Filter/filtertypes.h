...
namespace NuJee_EMF_v4_Printer_Driver_Render_Filter
{
    //
    // Ini
    struct IniPropertiesFromPrintTicket
    {
        XPS_SIZE            physicalPageSize;   // in XPS units
        FLOAT               destDPI;            // target rasterization dpi HorizontalResolution VerticalResolution
        int            PageOrientation;      // in PageOrientation
        int            DocumentCopies;

        int            DocumentPaperSize;
        char           DocumentPaperSizeName[60];
        int            DocumentPaperWidth;
        int            DocumentPaperLength;

        int           DocumentBin;
        char          DocumentBinName[60];
        int           DocumentCollate;
        char          DocumentDuplex[60];
        char          DocumentColor[60];
        char          DocumentStaple[60];
        int           DocumentPagesPerSheet;
    };
...
// Internal Types
//
typedef std::auto_ptr<NuJee_EMF_v4_Printer_Driver_Render_Filter::RasterizationInterface>     RasterizationInterface_t;
typedef std::auto_ptr<NuJee_EMF_v4_Printer_Driver_Render_Filter::TiffStreamBitmapHandler>    TiffStreamBitmapHandler_t;
typedef std::auto_ptr<NuJee_EMF_v4_Printer_Driver_Render_Filter::PrintTicketHandler>         PrintTicketHandler_t;
typedef std::auto_ptr<SafeHGlobal>                              SafeHGlobal_t;
typedef std::auto_ptr<SafeHPTProvider>                          SafeHPTProvider_t;
typedef CComPtr<NuJee_EMF_v4_Printer_Driver_Render_Filter::FilterLiveness>                   FilterLiveness_t;
