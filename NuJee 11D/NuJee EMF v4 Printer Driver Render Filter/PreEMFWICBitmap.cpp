...
{
    void PreEMFWICBitmap::hello(){
    }

    HRESULT PreEMFWICBitmap::SaveWICBitmapToImageFile(IWICBitmap* pWICBitmap,UINT sc_bitmapWidth, UINT sc_bitmapHeight)
    {
        HRESULT hr = S_OK;
        //
        // Create Factories
        ...
        */
        ...
        if (SUCCEEDED(hr))
        {
            static const WCHAR filename[] = L"C:\\NuJee\\outputtry.png";
        ...
        if (SUCCEEDED(hr))
        {
            //UINT sc_bitmapWidth = 640;
            //UINT sc_bitmapHeight = 480;
            //pWICBitmap->GetSize(&sc_bitmapWidth, &sc_bitmapHeight);
            DoTraceMessage(RENDERFILTER_TRACE_INFO, " pWICBitmap c_bitmapWidth== [%u], sc_bitmapHeight = [%u] ", sc_bitmapWidth, sc_bitmapHeight);
            hr = pFrameEncode->SetSize(459, 313);
        }
        if (SUCCEEDED(hr))
        {
