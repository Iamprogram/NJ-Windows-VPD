...
{                                                                                   \
    HRESULT hr_ = func_;                                                            \
    if (FAILED(hr_)) { NuJee_EMF_v4_Printer_Driver_Render_Filter::ThrowHRException(hr_, __FILE__, __LINE__); }   \
} 
...
#define CATCH_VARIOUS(hr_)                  \
    ...
    catch(NuJee_EMF_v4_Printer_Driver_Render_Filter::hr_error const& e)  \
    {                                       \
        hr_ = e.hr;                         \
    }                                       \
